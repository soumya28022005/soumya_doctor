...
app.get("/api/dashboard/:role/:userId", async (req, res) => {
    ...
    try {
        ...
        if (role === 'patient') {
            const appointmentsRes = await db.query("SELECT a.*, d.name as doctor_name, c.name as clinic_name FROM appointments a JOIN doctors d ON a.doctor_id = d.id JOIN clinics c ON a.clinic_id = c.id WHERE a.patient_id = $1 ORDER BY a.date DESC, a.time ASC", [userId]);
            ...
            data = { ...data, clinic: clinicRes.rows[0], doctors: clinicDocsRes.rows, allDoctors: allDocsRes.rows, joinRequests: requestsRes.rows, invitations: invitationsRes.rows, patients: patientsRes.rows };
        } else if (role === 'admin') {
             const today = new Date().toISOString().slice(0, 10);
